// https://github.com/g1eb/calendar-heatmap/blob/master/LICENSE
'use strict';
...
var calendarHeatmap = {
   ...
   * Initialize
   */
  init: function(data, container, color, overview, handler, valueType) {
    // Set calendar data
    ...
    // Set valueType
    calendarHeatmap.valueType = valueType || 'time';
  ...
  drawGlobalOverview: function() {
    ...
    var item_block = calendarHeatmap.items.selectAll('.item-block-year')
      ...
      .on('mouseover', function(d) {
        ...
        // Construct tooltip
        var tooltip_html = '';
        if (calendarHeatmap.valueType === 'time') {
          ...
          tooltip_html += `<div><span><strong>Total ${calendarHeatmap.valueType}:</strong></span>`;
  ...
  drawYearOverview: function() {
    ...
    calendarHeatmap.items.selectAll('.item-circle')
      ...
      .on('click', function(d) {
        // TODO: Redirect to photos on this date, but for now do nothing.
        return;
      ...
      .on('mouseover', function(d) {
        ...
        if (calendarHeatmap.valueType === 'time') {
          ...
          tooltip_html += `<div class="header"><strong>${d.total} ${calendarHeatmap.valueType}</strong></div>`;
          tooltip_html += '<div>on ' + moment(d.date).format('dddd, MMM Do YYYY') + '</div><br>';
        }
